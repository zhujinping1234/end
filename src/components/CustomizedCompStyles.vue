<script lang="ts">
  ...
  export default {
    ...
    setup(props: { rawCss: string }) {
      //  let compiledCss=ref('')

      //   watch(
      //   () =>props.rawCss,
      //   () => {
      //       if (props.rawCss && props.rawCss.length > 0) {
      //         const prefixedCss: any = prefixCss(props.rawCss)
      ...
      const compiledCss = computed(() => {
        if (props.rawCss && props.rawCss.length > 0) {
          const prefixedCss: string = prefixCss(props.rawCss)
