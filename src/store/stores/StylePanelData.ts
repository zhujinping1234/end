...
import { compStyle } from '../../model/AppInstanceModel'
export let styleStore = defineStore('main2', {
  state: (): compStyle => {
    return {
      StyleData: {
  ...
  actions: {
    setCompStyle(obj: { compID: string; compStyle: string }) {
      //    state.StyleData.forEach((item: any) => {
      //           if (item.comp_id == obj.compID) {
      //             item.style=obj.compStyle
      //             return
      //           }
      //         })
      Object.keys(this.StyleData).forEach((key) => {
        if (key == obj.compID) {
          this.StyleData[key] = obj.compStyle
          return
        }
      })
    }
